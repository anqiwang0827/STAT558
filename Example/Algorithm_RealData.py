""" This module implements 9 functions. The cust_func calculates the objective 
value of the cost function. The norm_func calculates the norm value. The 
computegrad function calculates gradient of cost funcation given beta values. 
The backtracking function returns step size by implementing backtracking rule. 
The funcation graddescent that implements the gradient descent algorithm with 
the backtracking rule to tune the step-size. The function fastgradalgo that 
implements the fast gradient algorithm. The objective_plot function plots the 
curve of the objective values for both algorithms versus the iteration counter t.
The compute_misclassification_error function calculates the misclassification 
error. And the plot_misclassification_error function will plot the curve of 
misclassification error values for both algorithms versus the iteration counter t.
    This module runs in the following order: Import libraries -> Load Data and split 
to training and test set -> Find optimal lambda value to optimize the algorithm -> 
Calculate beta values from both gradient algorithm and fast algorithm -> 
Plot objective value and missclassification error
...
data = data.values
n, k = data.shape #number of samples and dimension of each sample
X = data[:, 0:k-1]
y = data[:, k-1]
